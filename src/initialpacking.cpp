...
for(config.t=tstart;config.t<tend;config.t+=dt)   //start time loop
	{
		...
		{ 
			...
			if(config.cell.boundary != "HOLLOW_CYLINDER")
                volume *= 4.0 *PI /3.0 / (config.cell.L.x[0]*config.cell.L.x[1]*config.cell.L.x[2]);
            
            if(config.cell.boundary == "HOLLOW_CYLINDER")
                volume *= 4.0 /3.0 / (config.cell.Height*(config.cell.Rexternal*config.cell.Rexternal-config.cell.Rinternal*config.cell.Rinternal));
